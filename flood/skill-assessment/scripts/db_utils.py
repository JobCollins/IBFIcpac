...
def get_credentials(credentials_file='credentials.cfg'):
    """
    get credentials (username, password) and 
    database url from config file
    """
...
def get_glofas_data(country='kenya', return_type='csv', credentials_file='credentials.cfg'):
    """ 
    get glofas data of a country from database, return csv or xarray
    """

    username, password, url = get_credentials(credentials_file)
    engine = db.create_engine('postgresql://'+username+':'+password+'@'+url.replace('http://', '') +  '/cradatabase_staging')
    ...
    query = db.select([table]).where(table.columns.time.between(datetime.date(2000,1,1), datetime.date(2021, 10, 31)))
    ...
    if return_type == 'csv':
        ...
        print('Error: return type is not known, valid types are csv, xarray')